...
func (kvt *KVT) RangeQuery(db Poler, rangeInfo RangeInfo) (result []any, err error) {
	...
	pks, err := db.Query(index.path, prefix, filter) //query (key, pk) pair
	...
	for i := range pks {
		v, err := db.Get(kvt.path, pks[i].Value)
...
func (kvt *KVT) Gets(db Poler, prefix []byte) (result []any, err error) {
	...
	pks, err := db.Query(kvt.path, prefix, func([]byte) bool { return true }) //query (key, pk) pair
	if err != nil {
		return result, err
	}
